...
extern int nNewAreaFile;  // 1 = true, 0 = false
...
void load_mobiles( FILE *fp )
{
	...
	pMobIndex->race		 	= race_lookup(fread_string( fp ));
        ...
        if (IS_SET(pMobIndex->affected_by, AFF_BLINDFOLDED)) {
		bug( "Load_mobiles: vnum %d loading blindfolded.",vnum);
	}
	if (IS_SET(pMobIndex->affected_by,AFF_SUFFOCATING)) {
		REMOVE_BIT(pMobIndex->affected_by,AFF_SUFFOCATING);
		SET_BIT(pMobIndex->affected_by, AFF_INFRARED);
        }
	...
	/* read hit dice */
 	...
 	/* read endurance dice */
	pMobIndex->endurance[DICE_NUMBER]	= fread_number( fp );
					  fread_letter( fp );
	pMobIndex->endurance[DICE_TYPE]	= fread_number( fp );
					  fread_letter( fp );
	pMobIndex->endurance[DICE_BONUS]	= fread_number( fp );
	...
	/* read spheres from mobiles */
	if (pMobIndex->area->version > 1 ) {
	  pMobIndex->sphere[SPHERE_AIR]    = fread_number( fp );
	  pMobIndex->sphere[SPHERE_EARTH]  = fread_number( fp );
	  pMobIndex->sphere[SPHERE_FIRE]   = fread_number( fp );
	  pMobIndex->sphere[SPHERE_SPIRIT] = fread_number( fp );
	  pMobIndex->sphere[SPHERE_WATER]  = fread_number( fp );
	}
	
	/* read world */	
	if (pMobIndex->area->version > 1 ) {
	  pMobIndex->world = fread_flag( fp );
	}
...
/*	pMobIndex->size			= size_lookup(fread_word(fp)); */
	...
	letter = fread_letter( fp );
	if (letter == 'G') {
	  pMobIndex->guild_guard = clan_lookup(fread_string(fp));
	}
	else {
	  pMobIndex->guild_guard             = 0;
	  ungetc( letter, fp );
	}
	...
	for ( ; ; ) {
	  ...
	  if (letter == 'F') {
	    ...
	    else if (!str_prefix(word,"guard_flag"))
		 SET_BIT(pMobIndex->guild_guard_flags, vector);
	    else if (!str_prefix(word,"gain"))
		 SET_BIT(pMobIndex->gain_flags, vector);
	    else if (!str_prefix(word,"train"))
		 SET_BIT(pMobIndex->train_flags, vector);
	    ...
	    if (!str_prefix(word,"tlevel"))
		 pMobIndex->train_level = fread_number(fp);
...
void convert_mobile( MOB_INDEX_DATA *pMobIndex )
{
    ...
    pMobIndex->endurance[DICE_NUMBER]   = level;
    pMobIndex->endurance[DICE_TYPE]     = 10;
    pMobIndex->endurance[DICE_BONUS]    = 100;
...
void recalc( MOB_INDEX_DATA *pMob )
{
	...
	pMob->endurance[DICE_NUMBER]		= pMob->level;
	pMob->endurance[DICE_TYPE]		= 10 + (pMob->level / 8);
	pMob->endurance[DICE_BONUS]		= 100;

	if ( IS_SET(pMob->act, ACT_CLERIC)) 
		pMob->endurance[DICE_BONUS]	*= (1 + pMob->level / 3);
