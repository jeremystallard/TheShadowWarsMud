...
void save_mobile( FILE *fp, MOB_INDEX_DATA *pMobIndex )
{
    ...
    fprintf( fp, "%dd%d+%d ",      pMobIndex->endurance[DICE_NUMBER], 
    ...
    /* Save spheres for mobiles */
    fprintf(fp,"%d %d %d %d %d\n", pMobIndex->sphere[SPHERE_AIR],
		                         pMobIndex->sphere[SPHERE_EARTH],
		                         pMobIndex->sphere[SPHERE_FIRE],
                                   pMobIndex->sphere[SPHERE_SPIRIT],
                                   pMobIndex->sphere[SPHERE_WATER]);
    /* save world for mobiles */
    fprintf(fp, "%s\n", print_flags(pMobIndex->world));
    ...
    // Guild guards
    if (pMobIndex->guild_guard) {
	 fprintf( fp, "G %s~\n", clan_table[pMobIndex->guild_guard].name );
    }

    if ((temp = DIF(race_table[race].act,pMobIndex->act)))
     	fprintf( fp, "F act %s\n", fwrite_flag(temp, buf) );

    if (pMobIndex->guild_guard_flags != 0)
	 fprintf(fp, "F guard_flag %s\n", print_flags(pMobIndex->guild_guard_flags));

    if (pMobIndex->gain_flags != 0)
	 fprintf(fp, "F gain %s\n", print_flags(pMobIndex->gain_flags));
    if (pMobIndex->train_flags != 0) {
	 fprintf(fp, "F train %s\n", print_flags(pMobIndex->train_flags));
	 fprintf(fp, "V tlevel %d\n", pMobIndex->train_level);
    }
...
void save_object( FILE *fp, OBJ_INDEX_DATA *pObjIndex )
{
    ...
    fprintf( fp, "%s~\n",    !IS_NULLSTR(pObjIndex->material) ? pObjIndex->material : "");
  ...
  for( iHash = 0; iHash < MAX_KEY_HASH; iHash++ ) {
    for( pRoomIndex = room_index_hash[iHash]; pRoomIndex; pRoomIndex = pRoomIndex->next ) {
	 if ( pRoomIndex->area == pArea ) {
	   ...
	   if (IS_NULLSTR(pRoomIndex->name)) {
		sprintf(buf, "Room vnum %d have empty room name!", pRoomIndex->vnum);
		log_string(buf);
	   }
	   
	   fprintf( fp, "%s~\n",		!IS_NULLSTR(pRoomIndex->name) ?  pRoomIndex->name : "");
	   ...
	   for( i = 0; i < 10; i++ ) {
		...
		if ( pExit->u1.to_room ) {
		  ...
		  if ( IS_SET(pExit->rs_flags, EX_CLOSED)
			  ...
			  ||   IS_SET(pExit->rs_flags, EX_BARRED)
		  ...
		  fprintf( fp, "%s~\n",      !IS_NULLSTR(pExit->keyword) ? pExit->keyword : "");
	   ...
	   if (pRoomIndex->endurance_rate != 100 || pRoomIndex->heal_rate != 100)
		fprintf ( fp, "M %d H %d\n",pRoomIndex->endurance_rate,
...
void save_area( AREA_DATA *pArea )
{
  ...
  sprintf( buf, AREA_DIR "%s.bak", pArea->file_name );
  ...
  if ( !( fp = fopen( buf, "w" ) ) ) {
    ...
    /* Only save Flags for use. ADDED AND CHANGED are internal */
    int tempflags = pArea->area_flags;
    if (IS_SET(tempflags, AREA_ADDED))
	 REMOVE_BIT(tempflags, AREA_ADDED);
    if (IS_SET(tempflags, AREA_CHANGED))
	 REMOVE_BIT(tempflags, AREA_CHANGED);
    if (IS_SET(tempflags, AREA_LOADING))
	 REMOVE_BIT(tempflags, AREA_LOADING);
    if (IS_SET(tempflags, AREA_BUILDING))
	 REMOVE_BIT(tempflags, AREA_BUILDING);
    ...
    sprintf( buf2, AREA_DIR "%s", pArea->file_name );
    
    if ( rename( buf, buf2 ) != 0 ) {
	 bugf( "save_area : rename" );
	 perror(buf2);
    }
  }
...
/** Function: save_guilds
  ...
  */
void do_guildsave(CHAR_DATA *ch, char *argument)
{
  FILE *fp;
  int i;
  int value;
...
void do_sguildsave(CHAR_DATA *ch, char *argument)
{
  FILE *fp;
  int i;
  int value;
  ...
  if ( !( fp = fopen(buf, "w+" ) ) )
  {
     bug( "Open_sguild: fopen", 0 );
     return;
  }
  
  for (i=1; sguild_lookup(sguild_table[i].name) != 0; i++)
  {
    if ( !IS_SET(sguild_table[i].flags, SGUILD_DELETED) && sguild_lookup(sguild_table[i].name) != 0)
    {
      fprintf(fp, "\nGuild\t%s~\n", sguild_table[i].name);
      ...
      fprintf(fp, "Rooms\t%d\t%d\t%d\n", 
                   sguild_table[i].room[0],  
                   sguild_table[i].room[1],
                   sguild_table[i].room[2]);

      for (value=0; value < MAX_RANK; value++)
      {  
        if (sguild_table[i].rank[value].rankname == NULL ||
            sguild_table[i].rank[value].rankname[0] == '\0')
           sguild_table[i].rank[value].rankname = str_dup("Unassigned");
            
          fprintf(fp, "Rank\t%d\t%s~\n", 
                       value+1, 
                       sguild_table[i].rank[value].rankname);
      }
        
      for (value=0; value < MAX_RANK; value++)
        if (sguild_table[i].rank[value].skillname != NULL &&
            sguild_table[i].rank[value].skillname[0] != '\0')
          fprintf(fp, "Skill\t%d\t%s~\n", 
                       value+1, 
                       sguild_table[i].rank[value].skillname);
      ...
      /* Remove "changed" bit before writing flags to file */
      if (IS_SET(sguild_table[i].flags, SGUILD_CHANGED))
        REMOVE_BIT(sguild_table[i].flags, SGUILD_CHANGED);
        
      fprintf( fp, "Flags %s \n",  fwrite_flag( sguild_table[i].flags,  buf ) );
  ...
  fprintf(fp, "\nEnd\n");
  fclose(fp);
  send_to_char("SGuild data file (sguild.dat) written.\n\r", ch);
  return;        
}

void do_ssguildsave(CHAR_DATA *ch, char *argument)
{
  FILE *fp;
  int i;
  int value;
  ...
  for (i=1; ssguild_lookup(ssguild_table[i].name) != 0; i++)
  {
    if ( !IS_SET(ssguild_table[i].flags, SSGUILD_DELETED) && ssguild_lookup(ssguild_table[i].name) != 0)
    {
      ...
      fprintf(fp, "Rooms\t%d\t%d\t%d\n", 
                   ssguild_table[i].room[0],  
                   ssguild_table[i].room[1],
                   ssguild_table[i].room[2]);

      for (value=0; value < MAX_RANK; value++)
      {  
        if (ssguild_table[i].rank[value].rankname == NULL ||
            ssguild_table[i].rank[value].rankname[0] == '\0')
           ssguild_table[i].rank[value].rankname = str_dup("Unassigned");
            
          fprintf(fp, "Rank\t%d\t%s~\n", 
                       value+1, 
                       ssguild_table[i].rank[value].rankname);
      }
        
      for (value=0; value < MAX_RANK; value++)
        if (ssguild_table[i].rank[value].skillname != NULL &&
            ssguild_table[i].rank[value].skillname[0] != '\0')
          fprintf(fp, "Skill\t%d\t%s~\n", 
                       value+1, 
                       ssguild_table[i].rank[value].skillname);
  
      fprintf(fp, "ML\t%d  %d  %d  %d\n", 
                   ssguild_table[i].ml[0],
                   ssguild_table[i].ml[1],
                   ssguild_table[i].ml[2],
                   ssguild_table[i].ml[3]);
      
      /* Remove "changed" bit before writing flags to file */
      if (IS_SET(ssguild_table[i].flags, SSGUILD_CHANGED))
        REMOVE_BIT(ssguild_table[i].flags, SSGUILD_CHANGED);
        
      fprintf( fp, "Flags %s \n",  fwrite_flag( ssguild_table[i].flags,  buf ) );
  ...
  fprintf(fp, "\nEnd\n");
  fclose(fp);
  send_to_char("SSGuild data file (ssguild.dat) written.\n\r", ch);
  return;        
}
