...
do_write(CHAR_DATA *ch, char *argument)
{
  ...
  if (!strcmp(argument,"write")) {
      string_append(ch,&letter->extra_descr->description);
  }
  else
  {
...
void
do_readletter(CHAR_DATA *ch, char *argument)
{
  OBJ_DATA *letter;
...
void
do_sealletter(CHAR_DATA *ch, char *argument)
{
  OBJ_DATA *letter;
  char buffer[MAX_STRING_LENGTH];
