...
extern void 	wear_obj	args( ( CHAR_DATA *ch, OBJ_DATA *obj, bool fReplace, char * location) );
...
/* Check if victim know the weave and if he can slice it before */
/* any damage is done.                                          */
bool slice_weave( CHAR_DATA *ch, CHAR_DATA *vch, int sn)
{
  int i=0;
  long vch_flows=0;
  long ch_flows =0;
  ...
  // Main sphere check
  if (ch->main_sphere == high_sphere) {
    // If both have same main sphere, let flows determine, 
    // else add bonus to the main sphere
    if (high_sphere != vch->main_sphere)
	 ch_flows += high_sphere_value/5;
  }
  
  /* If person is sleeping. Can't slice while sleeping */
  if (IS_SLEEPING(vch)) {
   return FALSE;
  }

  /* If person is shielded, he can't slice anything             */
  if( IS_AFFECTED( vch, AFF_SHIELDED ) ) {
    return FALSE;
  }

  /* Is player is stilled...                                    */
  ...
  /* If Autoslice OFF                                           */
  if (!IS_NPC(vch)) {
     if (!IS_SET(vch->act,PLR_AUTOSLICE)) {
       return FALSE;
     }
   }

  /* If stronger in the flows making the weave, slice           */
  /* The more the victim is holding, the higher chance to slice */
  /* if holding, holding value give a 10% bonus                 */ 
  ch_skill  = get_skill(ch,sn);
  vic_skill = get_skill(vch,sn);
...
// Skill in slice also count
  ch_skill += (get_skill(ch, skill_lookup("slice")) / 3);
  vic_skill += get_skill(vch,skill_lookup("slice") );

  if (vic_skill > ((ch_skill * 5) / 4))  //if victim is signifcantly better in the skill
  {
     return TRUE;
  }

  if (vic_skill < ch_skill) {
	return FALSE;
  }

  //If victim knows the skill, give a chance, even if a small one
  if (vic_skill > ch_skill) {
	int chance = vic_skill - ch_skill;
	if (ch->sex != vch->sex) {
		chance = chance / 2;
	}
  ...
  //victim knows less, but may be stronger, so compare that next and give chance to slice
  if (vch_flows > ch_flows) {
	int chance = (vch_flows - ch_flows) / 10;
	if (ch->sex != vch->sex) {
		chance = chance / 2;
	}
        return (number_chance(chance));
  }

  if (vch->holding > 0)
    return (number_chance( (vch_flows + (vch->holding/10)) - ch_flows));
  else
    return (number_chance(vch_flows - ch_flows));
}
...
bool saves_spell( int level, CHAR_DATA *victim, int dam_type )
{
	...
	save = 50 + (victim->level + victim->pcdata->extended_level - level) * 5 - victim->saving_throw * 2;
    ...
    if (!IS_NPC(victim) && class_table[victim->class].fEndurance)
	save = 9 * save / 10;
...
/* for finding endurance costs -- temporary version */
int endurance_cost (CHAR_DATA *ch, int min_endurance, int level)
{
  if (ch->level + 2 == level)
    return 1000;
  return UMAX(min_endurance,(100/(2 + ch->level - level)));
}
...
*/
  ...
  */
  ...
  target_name = arg2; /* Target     */
  cast_arg2   = arg2; /* Extra arg1 */
  cast_arg3   = arg3; /* Extra arg2 */
   
  if ( arg1[0] == '\0' ) {
    send_to_char( "Weave which what where?\n\r", ch );
    return;
  }
  ...
  if (IS_LINKED(ch) && !check_can_lead_link(ch))
  {
	send_to_char("You are unable to lead the current link!\n\r",ch);
	return;
  }

  if (ch->daze > 0) {
    if (IS_SET(ch->merits, MERIT_CONCENTRATION) && number_chance(20)) {
	 send_to_char("{WYou are in daze, but manage to concentrate enough to focus and target the flows.{x\n\r", ch);
    }
    else if (number_chance(35)) {
	 send_to_char("{WYou are in daze, but manage to concentrate enough to focus and target the flows.{x\n\r", ch);
    }
    else {
	 send_to_char("{DYou can't concentrate enough while in a daze.{x\n\r", ch);
	 return;
    }
  }
  ...
  if ((sn = find_spell(ch,arg1)) < 1
	 ...
	 || (!IS_NPC(ch) && (ch->level < skill_table[sn].skill_level[ch->class]
					 || ch->pcdata->learned[sn] == 0))) {
    send_to_char( "You don't know any weave of that name.\n\r", ch );
    return;
  }
  
  if (IS_NPC(ch) && ( skill_table[sn].restriction == RES_NOGAIN
  ...
  /* Cost more for those who are inefficient */
  if (IS_SET(ch->flaws, FLAW_INEFFICIENT))
    endurance = endurance * 2;
    
  /* Cost less for those who are efficient   */
  if (IS_SET(ch->merits, MERIT_EFFICIENT))
    endurance = endurance/2;  
    ...
    */
    if ( arg2[0] == '\0' ) {
	 if ( ( victim = ch->fighting ) == NULL ) {
	   send_to_char( "Weave the flows on whom?\n\r", ch );
	   return;
	 }
    }
    else {
...
*/
  ...
  case TAR_CHAR_SELF:
    if ( arg2[0] != '\0' && !is_name( target_name, ch->name ) ) {
	 send_to_char( "You cannot weave this flow on another.\n\r", ch );
	 return;
    }
  ...
  case TAR_CHAR_OTHER:
    if ( arg2[0] == '\0') {
	 send_to_char( "You cannot weave this flow on yourself.\n\r", ch );
	 return;
    }
    else {
    	 if ( ( victim = get_char_room( ch, target_name ) ) == NULL ) {
  ...
  case TAR_OBJ_INV:
    if ( arg2[0] == '\0' ) {
	 send_to_char( "What should the flows be woven upon?\n\r", ch );
	 return;
    }
  ...
  case TAR_OBJ_CHAR_OFF:
    if (arg2[0] == '\0') {
	 if ((victim = ch->fighting) == NULL) {
	   send_to_char("Weave the flows on whom or what?\n\r",ch);
	   return;
	 }
  ...
  for (i = 0; i < MAX_SPHERE; i++) {
    if (skill_table[sn].spheres[i] > ch->perm_sphere[i]) {
	 sprintf(buf, "Your weave starts to form but dissolves for lack of strength in %s.\n\r", 
		    capitalize(sphere_table[i].name));
	 
	 /* Optional way to give message ? */
	 /* sprintf(buf, "You cannot safely handle that much %s.\n\r", capitalize(sphere_table[i].name)); */
	 send_to_char(buf, ch);
   ...
   /* Check Talent - if weave require talent*/
   if (skill_table[sn].restriction == RES_TALENT ) {
      if (skill_table[sn].talent_req != 0) {
       if (!IS_NPC(ch) && !IS_SET(ch->talents, skill_table[sn].talent_req)) {
         sprintf(buf, "You try to form the weave '%s', but the complex pattern of %s is to beyond you.\n\r", skill_table[sn].name, (char *)flow_text(sn, ch));
         send_to_char(buf, ch);
  ...
  if ( !IS_NPC(ch) && ch->endurance < endurance ) {
    send_to_char( "Your weave begins to form but dissolves for lack of endurance.\n\r", ch );
  ...
  if ( number_percent( ) > get_skill(ch,sn) ) {
    sprintf(buf, "You try to weave flows of %s into %s but fail.\n\r", (char *)flow_text(sn, ch), skill_table[sn].name);
    send_to_char(buf, ch);
    check_improve(ch,sn,FALSE,1);
    ch->endurance -= endurance / 2;
    
    sprintf(buf,"You sense $n trying to weave flows of %s.", (char *)flow_text(sn, ch));
    if (ch->sex == SEX_MALE)
    ...
    /* If channelers in same room, let them know cause they will see */
    for (vch = char_list; vch != NULL; vch = vch_next) {
	 ...
	 if (ch->in_room != vch->in_room)
	   continue;
	 if (!IS_NPC(vch) && vch->pcdata->learned[skill_lookup(ch->sex == SEX_MALE ? "seize" : "embrace")] < 1)
	   continue;
         if (!IS_SET(vch->chan_flags, CHAN_SEECHANNELING))
           continue;	   
	 if (!IS_NPC(vch) && vch->pcdata->learned[sn] > 0) {
	   sprintf(buf,"$n weave flows of %s into %s.", (char *) flow_text(sn, ch),
			 skill_table[sn].name);
	   act(buf,ch,NULL,vch,TO_VICT);
	 }
	 else {

	   sprintf(buf, "You sense $n weave flows of %s into a weave you don't recognize.", (char *)flow_text(sn ,ch));
	   act(buf, ch,NULL,vch,TO_VICT);

	   /* Learn weave by looking at others: only if a teachable weave */
	   /* and in same group and a weave within victimes level range   */
	   if (dice(get_curr_stat(vch,STAT_INT),5) >= number_percent() && (!IS_NPC(ch))) {

		if (skill_table[sn].skill_level[vch->class] <= vch->level && !IS_NPC(vch) &&
		    is_same_group(ch, vch)) {

		  /* Teachable */
		  if (skill_table[sn].restriction != RES_GRANTED &&
			 skill_table[sn].restriction != RES_NOTEACH &&
			 skill_table[sn].restriction != RES_TALENT ) {
		    if (IS_RP(ch) && IS_RP(vch)) {
  		       sprintf(buf,"By concentrating on $n, you learn to weave %s by combining flows of %s.",
			  	     skill_table[sn].name, (char *)flow_text(sn, ch));
		       act(buf,ch,NULL,vch,TO_VICT);
		       vch->pcdata->learned[sn] = 1;
		    }
		  }
		}
	   }
    ...
    /* Check slice if victim can slice the forthcomming weave             */
    if ( (skill_table[sn].target == TAR_CHAR_OFFENSIVE) ||
	    (skill_table[sn].target == TAR_CHAR_OTHER)) {
  	if (victim && foxhead_immune(victim, sn)) {
		send_to_char("Your weave has no affect on that person.\r\n",ch);
		return;
  	}
  
	 if (slice_weave(ch, victim, sn)) {
	   
	   /* Need to channel to be able to slice */
	   if (!IS_AFFECTED(victim, AFF_CHANNELING)) {
		do_function(victim, &do_channel, "");
	   }

	   /* Only slice if holding OP */
	   if (IS_AFFECTED(victim, AFF_CHANNELING)) {
		sprintf(buf, "$n {rslice{x your %s weave as you release it.", skill_table[sn].name);
		act( buf, victim, NULL, ch, TO_VICT );
  		WAIT_STATE(ch,skill_table[sn].beats);
	   
/* Current only PC and TARGET will see sliceing
	   sprintf(buf, "{r$n{r slice $N{r's %s weave as $e releases it.{x", skill_table[sn].name);
	   if (ch->sex == SEX_MALE)
		send_to_malchan(buf,victim);
	   else
		send_to_femalchan(buf,victim);
*/
	   
		sprintf(buf, "You {rslice{x $N's %s weave as $E releases it.{x", skill_table[sn].name);		
    ...
    /* Execute the weave */
    if (IS_NPC(ch)) // || class_table[ch->class].fEndurance) 
	 /* class has spells */
	 (*skill_table[sn].spell_fun) ( sn, ch->level, ch, vo,target);
    else
	 (*skill_table[sn].spell_fun) (sn, ch->level + ch->pcdata->extended_level, ch, vo,target);
    check_improve(ch,sn,TRUE,1);

    /* Add residue to Room */
    cast_rd.sn = sn;    
    cast_rd.duration = skill_table[sn].skill_level[ch->class]/2;
    if (cast_rd.duration < 1)
	 cast_rd.duration = 1;
    cast_rd.sex = ch->sex;
    cast_rd.world = ch->world;
    cast_rd.casterId = ch->id;
    cast_room = get_room_index(ch->in_room->vnum);
    residue_to_room(cast_room, &cast_rd);
  }

  if ((victim) && (IS_GHOLAM(victim)))
  {
...
void spell_armor( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
    ...
    if (get_curr_flows(ch) >= MAX_FLOWS) {
	 send_to_char("Your strenght prevent you from substain any more flows.", ch);
	 return;
    }
...
void spell_call_tning_OLD( int sn, int level,CHAR_DATA *ch,void *vo,int target)
{
    CHAR_DATA *vch;
    CHAR_DATA *vch_next;
    int dam;
    ...
    send_to_char( "Your lightning strikes your foes!\n\r", ch );
...
void spell_calm( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
    ...
    /* If spec_channeler_hard always work */
    if ( IS_NPC(ch) && ch->spec_fun == spec_lookup( "spec_channeler_hard" ))
      mlevel = 0;
...
void spell_chain_lightning(int sn,int level,CHAR_DATA *ch, void *vo,int target)
{
	     ...
	     tmp_vict = next_vict) 
	{
	  ...
	  if (!is_safe_spell(ch,tmp_vict,TRUE) && tmp_vict != last_vict && !is_same_group(ch,tmp_vict))
	  {
	    found = TRUE;
	    last_vict = tmp_vict;
...
/*
  if (!IS_RP(ch)) {
     send_to_char("You must be IC to do this!\n\r",ch);
     return;
  }
*/

  if (!IS_NPC(victim) && !IS_RP(victim)) {
    send_to_char("They must be IC to do this!\n\r",ch);
    return;
   }
  ...
  if ( victim == ch ) {
    ...
    if (!IS_NPC(victim) && !IS_SET(ch->talents, TALENT_COMPULSION)) {
       sprintf(buf, "You don't have the talent for setting the complex pattern of compulsion on PCs.\n\r");
       send_to_char(buf, ch);
       return;
    }
  modifier = skill_lookup("compulsion");
  ...
  if ((!IS_FORSAKEN(ch)) && (!IS_NPC(victim)) && ((number_percent() < (modifier / 2 )) || (saves_spell( level, victim, DAM_CHARM)))) {	
  	act( "You try to set the compulsion web on $N but $S mind is hard to bend.", ch, NULL, victim, TO_CHAR);
  	act( "You feel as if someone is trying to get inside your head!", ch, NULL, victim, TO_VICT);
  	return;
  }
  
  if (IS_AFFECTED(victim, AFF_CHARM)) {
   send_to_char("They are already under some sort of comulsion!\n\r", ch);
   return;
  }

  if (IS_AFFECTED(ch, AFF_CHARM)) {
   send_to_char("You are unable to get these flows combined just now.\n\r", ch);
   return;
  }
  
  if (!IS_FORSAKEN(ch) && saves_spell( level, victim, DAM_CHARM)) {
     act( "You try to set the compulsion web on $N but $S mind is hard to bend.", ch, NULL, victim, TO_CHAR);
     act( "You feel as if someone is trying to get inside your head!", ch, NULL, victim, TO_VICT);
     return;  	
  }
...
*/  
  ...
  act( "You feel an odd sensation of ease flow over you!", ch, NULL, victim, TO_VICT );

  if (ch != victim)
    act( "You feel the web of compulsion snap in place on $N.", ch,NULL,victim,TO_CHAR);
    
  // Log it
  if (ch != victim && !IS_NPC(victim)) {
     sprintf(buf, "$N put a web of compulsion on %s", victim->name);
     wiznet(buf, ch, NULL, WIZ_CHANNELING, 0, get_trust(ch));
  	
     sprintf(buf, "%s put a web of compulsion on %s", ch->name, victim->name);
     log_string(buf);	
  }
...
void spell_create_flame(int sn,int level,CHAR_DATA *ch,void *vo,int target)
{
  ...
  flame = create_object( get_obj_index( OBJ_VNUM_FLAME ), 0 );
  flame->level = ch->level;

  if (learned >= 85 && !IS_NULLSTR(cast_arg2)) {
    if (strstr(colorstrem(cast_arg2), "flame") == NULL &&
	   strstr(colorstrem(cast_arg2), "fire") == NULL) {
	 send_to_char("You must include one of the keywords 'flame' or 'fire' in the description.\n\r", ch);
	 return;
    }

    free_string(flame->short_descr);
    flame->short_descr = str_dup(cast_arg2);

    sprintf(buf, "%s hovers here, giving off a soft light.", capitalize(cast_arg2));
    free_string(flame->description);
    flame->description = str_dup(buf);
  }
    
  //flame->timer = get_level(ch);
  flame->timer = get_curr_op(ch)/10;
  
  obj_to_room( flame, ch->in_room );
    
  act( "The air shimmers before $n and $p appears.",  ch, flame, NULL, TO_ROOM );
  act( "The air shimmers before you and $p appears.", ch, flame, NULL, TO_CHAR );
  return;
}
...
void spell_continual_light(int sn,int level,CHAR_DATA *ch,void *vo,int target)
{
  ...
  light = create_object( get_obj_index( OBJ_VNUM_FLAME ), 0 );
...
void spell_control_weather(int sn,int level,CHAR_DATA *ch,void *vo,int target) 
{
  ...
  if ( !str_cmp( target_name, "better" ) )
    weather_info.change += g_FreezeWeather ? 0 : change;
  ...
  sprintf(buf, "Combining your flows into a complex pattern you aim a weave to make the weather %s.\n\r", target_name);
  send_to_char(buf, ch );
  return;
}
...
void spell_create_food( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
    ...
    mushroom = create_object( get_obj_index( OBJ_VNUM_BERRY ), 0 );
...
void spell_create_spring(int sn,int level,CHAR_DATA *ch,void *vo,int target)
{
  ...
  if (learned >= 85 && !IS_NULLSTR(cast_arg2)) {
    if (strstr(colorstrem(cast_arg2), "spring") == NULL &&
	   strstr(colorstrem(cast_arg2), "water") == NULL) {
	 send_to_char("You must include one of the keywords 'spring' or 'water' in the description.\n\r", ch);
	 return;
    }

    free_string(spring->short_descr);
    spring->short_descr = str_dup(cast_arg2);

    sprintf(buf, "%s flows from the ground here.", capitalize(cast_arg2));
...
void spell_dispel_evil( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
    ...
    if ( IS_GOOD(victim) )
    {
	act( "The Creator protects $N.", ch, NULL, victim, TO_ROOM );
	return;
    }
...
void spell_earthquake_OLD( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    CHAR_DATA *vch;
    CHAR_DATA *vch_next;
	...
	if ( vch->in_room == ch->in_room )
	{
	    if ( vch != ch && !is_safe_spell(ch,vch,TRUE) && !IS_IMMORTAL(vch))
...
void spell_enchant_armor( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
    ...
    //ALIGN_MATRIX talent?
    if (IS_SET(ch->talents, TALENT_ALIGHMATRIX)) {
      fail += 25;
    }
    ...
    {
	act("$p starts to take on the qualities of cuendillar, then disintegrates!",ch,obj,NULL,TO_CHAR);
	act("$p starts to take on the qualities of cuendillar, then disintegrates!",ch,obj,NULL,TO_ROOM);
    	damage( ch, ch, 1000, sn, DAM_FIRE ,TRUE);
	extract_obj(obj);
	return;
    }
...
void spell_enchant_weapon(int sn,int level,CHAR_DATA *ch, void *vo,int target)
{
    ...
    {
	act("$p shivers violently and {Rexplodes{x!",ch,obj,NULL,TO_CHAR);
	act("$p shivers violently and {Rexplodeds{x!",ch,obj,NULL,TO_ROOM);
	extract_obj(obj);
	return;
    }
	...
	act("$p {Yglows{x brightly, then fades...oops.",ch,obj,NULL,TO_CHAR);
	act("$p {Yglows{x brightly, then fades.",ch,obj,NULL,TO_ROOM);
	obj->enchanted = TRUE;
...
void spell_fireball_OLD( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    static const sh_int dam_each[] = 
    {
...
void spell_fireproof(int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
    ...
    act("$p glows for a moment.",ch,obj,NULL,TO_ROOM);
}
...
void spell_faerie_fire( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
  ...
  send_to_char("Disabled in respect to Robert Jordan\n\r", ch);
/*
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    AFFECT_DATA af;
...
void spell_faerie_fog( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
  ...
  for ( ich = ch->in_room->people; ich != NULL; ich = ich->next_in_room ) {
    ...
    act( "$n is revealed by {Gglowing{x dust!", ich, NULL, NULL, TO_ROOM );
    send_to_char( "You are revealed by {Gglowing{x dust!\n\r", ich );
  }  
  return;
}
...
void spell_fly( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
  ...
  /* Cannot be done to your self */
  if (victim == ch) {
    send_to_char("You cannot weave this flow on yourself.\n\r", ch);
    return;
  }

  //It is perfect IC to levitate someone during combat
  /*
  ...
  */
  ...
  if (number_percent() > chance)
  {
        send_to_char("You are unable to focus through the combat and levitate this target.\n\r",ch);
        return;
  }
  ...
  if ( IS_AFFECTED(victim, AFF_FLYING) ) {
    act("You start to combine the flows but realize $N is already levitated.",ch,NULL,victim,TO_CHAR);
    return;
  }
  ...
  send_to_char("Unseen bonds wraps around you and lift you up from the ground.\n\r", victim );
  act("$n's suddenly levitates from the ground.", victim, NULL, NULL, TO_ROOM );
  
  return;
}

void spell_fly_OLD( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
  CHAR_DATA *victim = (CHAR_DATA *) vo;
  AFFECT_DATA af;
...
void spell_gate_NOTINUSE( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    CHAR_DATA *victim;
    bool gate_pet;
...
void spell_harm_OLD( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
  char buf[MSL];
  CHAR_DATA *victim = (CHAR_DATA *) vo;
  int dam;
  ...
  sprintf(buf, "[ {YCoder{x ]: MAX DAM 'harm' = %d\n", dam);
  send_to_char(buf, ch);
...
void spell_harm( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
  ...
  if (!IS_NPC(ch)) {
    if (ch->main_sphere == SPHERE_SPIRIT) {
	 mainsphere = TRUE;
	 dam += ch->perm_sphere[SPHERE_SPIRIT]/2;
    }
  }
  ...
  if (IS_CODER(ch)) {
    sprintf(buf, "[ {YCoder{x ]: MAX DAM 'harm' = %d (%s)\n", dam, mainsphere ? "main sphere" : "not main sphere");
    send_to_char(buf, ch);
  }

  act("$n raises $s arm slightly and a instant {rpain{x shoot through your head!.", ch, NULL, victim, TO_VICT);
  act("You aim a serie of deadly flows of {Yspirit{x toward $N's head with {rHarm{x!.", ch, NULL, victim, TO_CHAR);
  act("$n raises $s arm slightly toward $N, and $E clutch $S head!", ch, NULL, victim, TO_NOTVICT);

  int minharms = 1;
  int maxharms = 4; //ch->pcdata->learned[sn] / 30; 
  if (!IS_NPC(ch))
  {
     if (ch->pcdata->learned[sn] / 30 > 8)
     {
	   minharms = (ch->pcdata->learned[sn] / 30) - 8;
	   if (minharms > 8) 
	      minharms = 8;
     }
     maxharms = ch->pcdata->learned[sn] / 30; 
  }
  else
  {
	minharms = 3;
  ...
  if (!IS_NPC(ch)) 
  {
    harms = number_range(minharms, maxharms);
  }
  else {
    harms = 1;
  }
...
void spell_heal_OLD( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
   CHAR_DATA *victim = (CHAR_DATA *) vo;
...
void spell_identify( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
  ...
  case ITEM_POTION:
  case ITEM_PILL:
    sprintf( buf, "Level {y%d{x weave of:", obj->value[0] );
    send_to_char( buf, ch );
  ...
  // If keeping talent and know the weave, show timer info
  if (IS_SET(ch->talents, TALENT_KEEPING) && get_skill(ch, gsn_keep) > 0 && obj->timer > 0) {
    sprintf(buf, "{DPerish{x in {y%d{x hours.\n\r", obj->timer);
    send_to_char(buf, ch);
  }
...
{
    send_to_char( "That's not a weave!\n\r", ch );
    return;
}
...
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    victim->endurance = UMIN( victim->endurance + level, victim->max_endurance );
    if (victim->max_endurance == victim->endurance)
...
void spell_sanctuary( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
    ...
    affect_to_char( victim, &af );
    act( "$n is surrounded by a shield of air and spirit.", victim, NULL, NULL, TO_ROOM );
    send_to_char( "You are surrounded by a shield of air and spirit.\n\r", victim );
    return;
}
...
void spell_sleep( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
    ...
    if (!IS_NPC(victim) && !IS_SET(ch->talents, TALENT_COMPULSION)) {
       sprintf(buf, "You don't have the talent for setting the complex pattern of %s on PCs.\n\r", (char *)flow_text(sn, ch));
       send_to_char(buf, ch);
       return;
    }
...
void spell_teleport( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    ...
    pRoomIndex = get_random_room(victim, TRUE);
...
char * flow_text(sh_int sn, CHAR_DATA *ch)
{
  ...
  /* Count how many flows make this weave*/
  for (i = 0; i < MAX_SPHERE; i++) {
    if (skill_table[sn].spheres[i] > 0)
	 cnt++;
  }
  ...
  for (i = 0; i < MAX_SPHERE; i++) {
    if (skill_table[sn].spheres[i] > 0) {
	 ...
	 if ( (cnt == cnt2) && (cnt > 1))
	   strcat(buf, " and ");
	 if ( (cnt2 < cnt) && (cnt > 0) && (cnt != 2)  && (cnt2 > 1) && (i > 0))
	   strcat(buf, ", ");
	 strcat(buf, capitalize(sphere_table[i].name));
    }
  }

  /* If no flows, make a default message */
  if (cnt == 0)
    sprintf(buf, "the one power");
