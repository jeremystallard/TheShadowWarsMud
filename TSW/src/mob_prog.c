 ...
 *----------------------------------------------------------------------
 */
int cmd_eval( unsigned int vnum, char *line, int check,
	    ...
	    case 'N': i=someone;
		    ...
		    if ( ch != NULL && can_see( mob, ch ) ) {
			 if (IS_NPC(ch))
			   ...
			   i = ch->pcdata->appearance;
		    }								    
		    break;

	   case 'D':
		i = (ch != NULL && can_see(mob, ch)) ?
		  (IS_NPC(ch) ? ch->short_descr : ch->pcdata->appearance) : someone;
		break;
            case 't': 
		i = someone;
...
#define MAX_CALL_LEVEL    6 /* Maximum nested calls */
...
void program_flow( unsigned int pvnum, 
			    char *source, 
			    CHAR_DATA *mob, 
			    CHAR_DATA *ch, 
			    const void *arg1, 
			    ...
			    int numlines )	
{
    ...
    unsigned int mvnum = mob->pIndexData->vnum;

    //if( ++call_level > MAX_CALL_LEVEL )
    if( numlines == 1 && ++call_level > MAX_CALL_LEVEL )
    {
        sprintf(buf, "[*****] BUG: MOBprogs: MAX_CALL_LEVEL exceeded, mvnum=%d  pvnum=%d", mob->pIndexData->vnum, pvnum );
        log_string(buf);
	//bug( "MOBprogs: MAX_CALL_LEVEL exceeded, mvnum=%d  pvnum=%d", mob->pIndexData->vnum, pvnum );
	--call_level;
	return;
    }
	...
	const void *arg1, const void *arg2, int type )
{
	...
	&&  strstr( argument, prg->trig_phrase ) != NULL )
        {
	    char buf[256];
            sprintf(buf,"Program %d on Act Trigger",prg->vnum);
            log_string( buf );

		for(test = first_sleep; test != NULL; test = test->next)
		  if(test->ch == ch && test->vnum == prg->vnum)
		    return;
	    program_flow( prg->vnum, prg->code, mob, ch, arg1, arg2, 1 );
	    break;
	}
    }
    return;
	...
	const void *arg1, const void *arg2, int type )
{
	...
	&&   number_percent() < atoi( prg->trig_phrase ) )
        {
//	    char buf[256];
//            sprintf(buf,"Program %d on Percent Trigger",prg->vnum);
//            log_string( buf );
...
void mp_bribe_trigger( CHAR_DATA *mob, CHAR_DATA *ch, int amount )
{
	...
	{
//	    char buf[256];
//            sprintf(buf,"Program %d on Bribe Trigger",prg->vnum);
//            log_string( buf );

	  for(test = first_sleep; test != NULL; test = test->next)
	    if(test->ch == ch && test->vnum == prg->vnum)
		 return;
	  program_flow( prg->vnum, prg->code, mob, ch, NULL, NULL, 1 );
	  break;
	}
    }
    return;
...
bool mp_exit_trigger( CHAR_DATA *ch, int dir )
{
	...
	&&   ( HAS_TRIGGER(mob, TRIG_EXIT) || HAS_TRIGGER(mob, TRIG_EXALL) ) )
	{
		...
		{
//	    		char buf[256];
//            		sprintf(buf,"Program %d on Exit Trigger",prg->vnum);
//            		log_string( buf );
...
void mp_give_trigger( CHAR_DATA *mob, CHAR_DATA *ch, OBJ_DATA *obj )
{
	...
	if ( prg->trig_type == TRIG_GIVE )
	{
		...
		{
//	    		char buf[256];
//            		sprintf(buf,"Program %d on Give Trigger",prg->vnum);
//            		log_string( buf );
		  ...
		  for(test = first_sleep; test != NULL; test = test->next)
		    ...
		    program_flow(prg->vnum, prg->code, mob, ch, (void *) obj, NULL, 1);
		    return;
		}
	    }
	    /*
...
void mp_hprct_trigger( CHAR_DATA *mob, CHAR_DATA *ch )
{
	...
	{
//	    char buf[256];
//            sprintf(buf,"Program %d on hprct Trigger",prg->vnum);
//            log_string( buf );
