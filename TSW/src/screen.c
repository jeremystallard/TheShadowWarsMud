...
{
	sh_int * dice = (sh_int *) point;
static	char buf[30];

	sprintf(buf, "%dd%d+%d", dice[DICE_NUMBER], dice[DICE_TYPE], dice[DICE_BONUS] );
...
const struct olc_show_table_type redit_olc_show_table [] =
{
	...
	{
		"endurance", &xRoom.endurance_rate, "Endurance:", OLCS_SHINT,
		10, 2, 3, 1, 1, NULL
	},
	{
...
const struct olc_show_table_type medit_olc_show_table [] =
{
	...
	{
		"level", &xMob.level, "Level :", OLCS_SHINT,
		1, 2, 2, 1, 1, NULL
	},
	{
	...
	{
		"endurance", &xMob.endurance, "EnduranceD:", OLCS_STRFUNC,
