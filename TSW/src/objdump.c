...
const char * wear_name (unsigned int);
...
const char * item_type (unsigned int);

 This version of item_type_name does the same, but takes just a unsigned int as a
 parameter - the handler.c:item_type_name takes an OBJ_DATA*, and what I 
 needed was to find the type name of a OBJ_INDEX_DATA*
 
int get_affect_total (AFFECT_DATA*, unsigned int);

 Returns the total modifier of a certain affect type in this linked list
 of AFFECT_DATAs.
 
bool is_affect (AFFECT_DATA*, unsigned int);
...
printobj (FILE*, OBJ_RESET*, where, unsigned int, unsigned int, unsigned int, unsigned int, unsigned int);
...
void add_to_list (OBJ_RESET *list, unsigned int vnum, unsigned int level)
{
	OBJ_RESET * new_item;
...
bool is_in_list (OBJ_RESET *list, unsigned int vnum, unsigned int level)
{
...
char *item_index_type_name (unsigned int item_type)
{
	switch (item_type)
	{
    ...
    case ITEM_PROTECT:		return "protect";
    ...
    case ITEM_AIRWALL:		return "AIRWALL";
...
void printobj (FILE *fp, OBJ_RESET *list, where location, unsigned int vnum_what, 
               unsigned int vnum_where, unsigned int vnum_who, unsigned int vnum_inside,
               bool reboot)
{
...
case inside:    /* inside is used. where is derived */
{
	...
	obj_inside = get_obj_type (obj_inside_index,0);
...
case ITEM_TATTOO: /* armor, ACs and bulk */
...
void printarea (FILE *fp, AREA_DATA *pArea)
{
  ...
  for( iHash = 0; iHash < MAX_KEY_HASH; iHash++ ) {
	...
	/* run through all the resets in that Room*/
	for (pReset = pRoomIndex->reset_first; pReset != NULL; pReset = pReset->next)
	{
    	...
    	default: break;/* uninteresting: exits */
    	} /* case */    	
	} /* for pReset */
     }
   }
