...
void free_room_index( ROOM_INDEX_DATA *pRoom )
{
    ...
    for ( pResidue = pRoom->residues; pResidue; pResidue = pResidue->next) {
	 free_residue(pResidue);
    }

    for (pWard = pRoom->wards; pWard; pWard = pWard->next) {
	 free_ward(pWard);
    }
    
    for (pWeave = pRoom->weaves; pWeave; pWeave = pWeave->next) {
	 free_room_weave(pWeave);
    }
...
MOB_INDEX_DATA *new_mob_index( void )
{
    ...
    pMob->endurance[DICE_NUMBER]	=   0;   /* ROM patch -- Hugin */
    pMob->endurance[DICE_TYPE]	=   0;   /* ROM patch -- Hugin */
    pMob->endurance[DICE_BONUS]	=   0;   /* ROM patch -- Hugin */
