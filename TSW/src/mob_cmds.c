...
void do_unchannel args((CHAR_DATA *ch, char *argument));
...
void do_mpkill( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ( victim = get_introname_char_room( ch, arg ) ) == NULL )
	return;
...
void do_mpdamage( CHAR_DATA *ch, char *argument )
{
  ...
  argument = one_argument( argument, damagetype);
  ...
  if (victim) {
    ...
    sprintf(buffer,"MpDamage - Victim set for %s",victim->name);
    flog( buffer);
    sprintf(buffer,"MpDamage - DamageType set for %s",damagetype);
    flog( buffer );
  }
...
/*
 * Sets a pause during MOBprogram execution. When the pause time expires,
 * the mobile moves on to the next command in the program
 * Pause is counted in PULSE_MOBILE [4 pulses per second]
 *
 * Syntax: mob pause [pulses]
 */
void do_mppause( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !is_number( arg ) )
    {
	bug( "MpPause: invalid arg from vnum %d.", 
		IS_NPC(ch) ? ch->pIndexData->vnum : 0 );
	return;
    }
    ch->mprog_delay = atoi( arg );
    DAZE_STATE(ch,atoi(arg));
}
...
void do_mpcall( CHAR_DATA *ch, char *argument )
{
    ...
    OBJ_DATA *obj1, *obj2;
    MPROG_CODE *prg;
    //extern void program_flow( unsigned int, char *, CHAR_DATA *, CHAR_DATA *, const void *, const void * );
    extern void program_flow(unsigned int, char *, CHAR_DATA *, CHAR_DATA *, const void *, const void *, int );
